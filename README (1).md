...
**6. Run it once.** Actions tab → Update standings → Run workflow.
...
1. Pulls Nebraska's schedule, scores, logos, and team colours from ESPN's
   public JSON feed.
2. Ignores any game that isn't final, or that finished less than
   `settleHours` (6) ago — it shows as *result under review* but doesn't count
   yet. This is the buffer against scoring corrections and mid-game hiccups.
...
- ESPN's JSON feed is public but unofficial and can change without warning. If
  it breaks, the overrides above will carry you through the season.
...
- `settleHours` controls the delay before a result counts. Set it to `0` to see
  results the moment ESPN calls them final.
